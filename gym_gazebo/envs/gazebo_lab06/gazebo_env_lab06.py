...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        pixel_count = [sum(sum(x) for x in thresh[:, i*slice_width:(i+1)*slice_width]) for i in range(10)]

        if not np.any(pixel_count[2:8]):
            self.timeout+=1
            if self.timeout > 30:
                done = True
        else:
            max_section = np.argmax(pixel_count)
            state[max_section] = 1
            self.timeout = 0

        cv2.imshow("Grayscale window", thresh)
        cv2.waitKey(3)
        print(state)
    ...
    def step(self, action):
        ...
        if vel_cmd.linear.x < 1e-6:
            self.wiggle_timeout+=1
        else:
            self.wiggle_timeout = 0
        if self.wiggle_timeout > 50:
            done = True
